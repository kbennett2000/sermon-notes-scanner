...
/**
 * Instrumented test that simulates the full OCR export flow using the same logic as the app:
 * 1. Loads an image from test_jpegs assets (simulating CameraFragment capture)
 * 2. Uses the image as-is (simulating CropFragment - no actual cropping needed for this test)
 * 3. Runs OCR with German language using app's OCR mode settings (simulating OCRFragment)
 * 4. Exports OCR text to TXT file with include_ocr option (simulating ExportFragment)
 * 5. Compares the resulting TXT file with the expected DOC_20251223_101951.txt
 * <p>
 * This test uses the same SharedPreferences-based OCR mode selection as OCRFragment,
 * ensuring that the test validates the actual app behavior without test-specific overrides.
 */
...
public class OcrExportFlowInstrumentedTest {
    ...
    public void testOcrExportFlow_GermanLanguage_CompareTxtOutput() throws Exception {
        ...
        assertTrue("Test image should have valid dimensions", testImage.getWidth() > 0 && testImage.getHeight() > 0);

        // Step 2: Image is used as-is (simulating CropFragment - the test image is already a document scan)
        ...
        try {
            ...
            System.out.println("[DEBUG_LOG] OCR mode from SharedPreferences: " + ocrMode +
            ...
            System.out.println("[DEBUG_LOG] Expected text (first 500 chars): " + normalizedExpected.substring(0, Math.min(500, normalizedExpected.length())));
            System.out.println("[DEBUG_LOG] Actual text (first 500 chars): " + normalizedActual.substring(0, Math.min(500, normalizedActual.length())));

            // Assert that similarity is above threshold (OCR may have minor errors)
            // Using 70% threshold to account for OCR variations
            assertTrue("OCR text should be at least 70% similar to expected text. Actual similarity: " + (similarity * 100) + "%",
                    similarity >= 0.70);

        } finally {
            ocrHelper.shutdown();
            if (preprocessedImage != null && preprocessedImage != croppedImage && !preprocessedImage.isRecycled()) {
    ...
    /**
     * Gets the selected OCR mode from SharedPreferences.
     * This is the same logic as OCRFragment.getSelectedOcrMode().
     *
    ...
    /**
     * Test that compares all three OCR modes (Original, Quick, Robust) and outputs
     * the similarity results for each mode.
     */
    @Test
    public void testCompareAllOcrModes() throws Exception {
        ...
        for (int ocrMode = 0; ocrMode <= 2; ocrMode++) {
            System.out.println("[DEBUG_LOG] ");
            System.out.println("[DEBUG_LOG] --- Testing OCR Mode " + ocrMode + " (" + modeNames[ocrMode] + ") ---");
            ...
            try {
                ...
                ocrHelper.setLanguage(OCR_LANGUAGE);
                boolean initialized = ocrHelper.initTesseract();
                assertTrue("Tesseract should be initialized for mode " + modeNames[ocrMode],
                ...
                if (preprocessedImage == null) {
                    System.out.println("[DEBUG_LOG] ERROR: Preprocessed image is null for mode " + modeNames[ocrMode]);
                ...
                if (ocrResult == null || ocrResult.text == null || ocrResult.text.isEmpty()) {
                    System.out.println("[DEBUG_LOG] ERROR: OCR result is empty for mode " + modeNames[ocrMode]);
                    similarities[ocrMode] = 0.0;
                    continue;
                }
                ...
                System.out.println("[DEBUG_LOG] First 300 chars: " + normalizedActual.substring(0, Math.min(300, normalizedActual.length())));
        ...
        System.out.println("[DEBUG_LOG] ");
        for (int i = 0; i <= 2; i++) {
            System.out.println("[DEBUG_LOG] " + modeNames[i] + ": " + String.format("%.2f", similarities[i] * 100) + "% similarity");
        }
        System.out.println("[DEBUG_LOG] ");
        ...
        System.out.println("[DEBUG_LOG] BEST MODE: " + modeNames[bestMode] + " with " +
                String.format("%.2f", similarities[bestMode] * 100) + "% similarity");
        System.out.println("[DEBUG_LOG] ========================================");
        ...
        assertTrue("At least one OCR mode should achieve >= 70% similarity. Best: " +
                String.format("%.2f", maxSimilarity * 100) + "%", maxSimilarity >= 0.70);
    }
}
